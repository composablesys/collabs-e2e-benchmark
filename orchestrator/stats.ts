...
// Importing this wasn't working for some reason...
type ContainerStats = any;
...
export const readStats = (
  ...
  writeStream.write(
    "Time, CPU (ns), CPU (%), MEM (MB), MEM (%), RECEIVE (KB), SEND (KB)\n"
  );
  ...
  readStream.on("data", (chunk: string) => {
    try {
      ...
      writeStream.write(
        `${getTime(stats)}, ${getCPUTime(stats)}, ${getCPUPercent(
          stats
        ).toFixed(2)}, ${getMemoryUsage(stats).toFixed(2)}, ${getMemoryPercent(
          stats
        ).toFixed(2)}, ${getNetworkReceivedUsage(stats).toFixed(
          2
        )}, ${getNetworkSendUsage(stats).toFixed(2)}\n`
      );
    } catch {
      // ignore
    }
